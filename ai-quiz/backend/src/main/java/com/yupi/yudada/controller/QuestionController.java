...
public class QuestionController {
    ...
    public SseEmitter aiGenerateQuestionSSE(AiGenerateQuestionRequest aiGenerateQuestionRequest, HttpServletRequest request) {
        ...
        sseEmitter.onTimeout(() -> {
            log.warn("SSE连接超时, appId={}", appId);
            sseEmitter.complete();
        });
        sseEmitter.onCompletion(() -> log.info("SSE连接完成, appId={}", appId));
        // AI 生成，SSE 流式返回
        Flowable<AiManager.StreamChunk> streamFlowable = aiManager.doStreamRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage, null);
        ...
        streamFlowable
                .observeOn(scheduler)
                .map(streamChunk -> streamChunk.getContent())
        ...
        streamFlowable
                ...
                .doOnNext(c -> {
                    ...
                    if (c == '}') {
                        ...
                        if (counter.get() == 0) {
                            // 输出当前线程的名称
                            log.info("AI generate thread: {}", Thread.currentThread().getName());
                            // 模拟普通用户阻塞
                            if (!isVip) {
                                Thread.sleep(10000L);
                            }
