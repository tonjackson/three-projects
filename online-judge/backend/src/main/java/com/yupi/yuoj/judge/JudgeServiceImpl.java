...
public class JudgeServiceImpl implements JudgeService {
    ...
    @Value("${codesandbox.auth.header:auth}")
    ...
    public void init() {
        CodeSandboxFactory.setAuthConfig(authHeader, authSecret);
    }
    ...
    public QuestionSubmit doJudge(long questionSubmitId) {
        ...
        // 3）更改判题（题目提交）的状态为 "判题中"，防止重复执行（CAS幂等控制）
        QuestionSubmit questionSubmitUpdate = new QuestionSubmit();
        questionSubmitUpdate.setId(questionSubmitId);
        questionSubmitUpdate.setStatus(QuestionSubmitStatusEnum.RUNNING.getValue());
        // 使用LambdaUpdateWrapper实现CAS：只有status=WAITING时才能更新为RUNNING
        com.baomidou.mybatisplus.core.conditions.update.LambdaUpdateWrapper<QuestionSubmit> updateWrapper =
