...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
     ...
     * 盐值，混淆密码
     */
    @org.springframework.beans.factory.annotation.Value("${user.password.salt:yuoj-default-salt-2024}")
    private String salt;
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        // 3. 记录用户的登录态
        request.getSession().setAttribute(USER_LOGIN_STATE, this.getLoginUserVO(user));
    ...
    public User getLoginUser(HttpServletRequest request) {
        LoginUserVO loginUserVO = getSessionLoginUserVO(request);
    ...
    private LoginUserVO getSessionLoginUserVO(HttpServletRequest request) {
        Object userObj = request.getSession().getAttribute(USER_LOGIN_STATE);
        if (userObj instanceof LoginUserVO) {
            return (LoginUserVO) userObj;
        }
        if (userObj instanceof User) {
            return this.getLoginUserVO((User) userObj);
        }
        return null;
    }
